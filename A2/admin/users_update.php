...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "UPDATE users SET username='$username', email='$email', password='$password', payment_method='$payment_method', shipping_address='$shipping_address' WHERE id_inUser=$id";
...
if($_SERVER["REQUEST_METHOD"] == "GET"){
    $id = $_GET['id'];
    $sql = "SELECT * FROM users WHERE id_inUser = $id";
        ...
        <div class="col-md-6">
            ...
            <form method="post" action="users_update.php">
                <div class="mb-3">
                    ...
                    <input type="hidden" name="id" value="<?php echo $user['id_inUser']; ?>">
                    <input type="text" class="form-control" value="<?php echo $user['username']; ?>"  id="username" name="username" required>
                </div>
                <div class="mb-3">
                    <label for="email" class="form-label">Email:</label>
                    <input type="text" class="form-control" id="email" value="<?php echo $user['email']; ?>"   name="email" required>
                </div>
                <div class="mb-3">
                    <label for="password" class="form-label">Password:</label>
                    <input type="text" class="form-control" id="password" value="<?php echo $user['password']; ?>"   name="password" required>
                </div>
                <div class="mb-3">
                    <label for="payment_method" class="form-label">Payment Method:</label>
                    <input type="text" class="form-control" id="payment_method" value="<?php echo $user['payment_method']; ?>"   name="payment_method" required>
                </div>
                
                <div class="mb-3">
                    <label for="shipping_address" class="form-label">Shipping Address:</label>
                    <input type="text" class="form-control" id="shipping_address" value="<?php echo $user['shipping_address']; ?>"   name="shipping_address" required>
                </div>
